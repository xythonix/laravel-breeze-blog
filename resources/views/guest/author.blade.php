...
@section('header')
    <div class="header flex-column justify-content-center align-items-start" >
        <h1 class="text-4xl font-bold text-white">Author - '{{$author_name}}'</h1>
    </div>
@endsection
	...
	<div class="container p-6 mx-auto space-y-8">
		...
		<div class="grid grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 lg:grid-cols-4">
			...
			@foreach ($authors as $author)
                @for ($i = 0; $i < count($author->Post); $i++)
                    <article class="flex flex-col shadow-sm bg-white">
                        @if ($author->post[$i]->post_img)
                            <img alt="" class="object-cover border-bottom w-full h-52 dark:bg-gray-500" src="{{asset('storage/images/'.$author->post[$i]->post_img)}}">
                        @else 
                        ...
                        <div class="flex flex-col flex-1 p-6">
                            <h3 class="flex-1 py-2 text-lg font-semibold leadi">
                                <a class="hover:underline" href="{{route('guest.singlePost',$author->post[$i]->id)}}">{{Str::limit($author->post[$i]->title,50)}}</a>
                            </h3>
                            <div class="flex flex-wrap justify-between pt-3 space-x-2 text-xs dark:text-gray-400">
                                <span class="text-gray-400">
                                    
                                </span>
                                <span class="font-semibold text-gray-500">{{$author->post[$i]->created_at->format('d M Y')}}</span>
                            </div>
                        </div>
                    </article>
                @endfor
